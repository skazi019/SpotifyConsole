...
def spotify_callback(request):
    ...
    t = SpotifyTokens.objects.create(sesssionkey=request.session.session_key,
                                     access_token=access_token, refresh_token=refresh_token, expires=expiry)
...
def user_profile(request):
    if not request.session.exists(request.session.session_key):
        return redirect('login')
    try:
        token = SpotifyTokens.objects.get(
