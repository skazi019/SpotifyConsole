...
def login(request):
    ...
    login_url = requests.Request('GET', "https://accounts.spotify.com/authorize", params={
        "client_id" : CLIENT_ID,
...
def spotify_callback(request):
    ...
    token_url = requests.post("https://accounts.spotify.com/api/token", data={
    ...
    top_artists_short, top_artists_medium, top_artists_long = get_top_artists(access_token=access_token)
    top_tracks_short, top_tracks_medium, top_tracks_long = get_top_tracks(access_token=access_token)
...
def get_user_profile_data(access_token):
    userProfile = requests.get("https://api.spotify.com/v1/me",
    headers={
...
def get_top_artists(access_token, limit=20):
    top_artists_short = json.loads(requests.get(f"https://api.spotify.com/v1/me/top/artists?time_range=short_term&limit={limit}",
    headers={
    "Content-Type" : "application/json",
    "Authorization" : f"Bearer {access_token}"
    }).text)
    top_artists_medium = json.loads(requests.get(f"https://api.spotify.com/v1/me/top/artists?time_range=medium_term&limit={limit}",
    headers={
    "Content-Type" : "application/json",
    "Authorization" : f"Bearer {access_token}"
    }).text)
    top_artists_long = json.loads(requests.get(f"https://api.spotify.com/v1/me/top/artists?time_range=long_term&limit={limit}",
    headers={
    "Content-Type" : "application/json",
    "Authorization" : f"Bearer {access_token}"
    }).text)
...
def get_top_tracks(access_token, limit=20):
    top_tracks_short = json.loads(requests.get(f"https://api.spotify.com/v1/me/top/tracks?time_range=short_term&limit={limit}",
    headers={
    "Content-Type" : "application/json",
    "Authorization" : f"Bearer {access_token}"
    }).text)
    top_tracks_medium = json.loads(requests.get(f"https://api.spotify.com/v1/me/top/tracks?time_range=medium_term&limit={limit}",
    headers={
    "Content-Type" : "application/json",
    "Authorization" : f"Bearer {access_token}"
    }).text)
    top_tracks_long = json.loads(requests.get(f"https://api.spotify.com/v1/me/top/tracks?time_range=long_term&limit={limit}",
    headers={
    "Content-Type" : "application/json",
    "Authorization" : f"Bearer {access_token}"
    }).text)
...
def artist_page(request):
    ...
    return render(request,"user_analytics/artists.html", {'artists_data_short' : artists_data_short, 'artists_data_medium' : artists_data_medium, 'artists_data_long' : artists_data_long})
...
def track_page(request):
    ...
    return render(request,"user_analytics/tracks.html", {'tracks_data_short' : tracks_data_short, 'tracks_data_medium' : tracks_data_medium, 'tracks_data_long' : tracks_data_long})
...
def recent_page(request):
    ...
    recent_tracks = json.loads(requests.get(f"https://api.spotify.com/v1/me/player/recently-played?limit=30",
    headers={
    "Content-Type" : "application/json",
    "Authorization" : f"Bearer {access_token}"
    }).text)['items']

    return render(request, "user_analytics/recent_tracks.html", {'recent_tracks' : recent_tracks})
...
def get_album(request, id):
    ...
    albumData = json.loads(requests.get(f"https://api.spotify.com/v1/albums/{id}",
    headers={
    "Content-Type" : "application/json",
    "Authorization" : f"Bearer {access_token}"
    }).text)
